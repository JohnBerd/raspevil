...
def shell_setup(encoded_command)
  ...
  File.open(File.expand_path('~') + '/raspevil/backdoor/shell.txt', 'w') do |f|
    f.write(s)
  end
...
def arduino_setup(host)
  ...
  s << "}"
  File.open(File.expand_path('~') + '/raspevil/sketch/sketch.ino', 'w') do |f|
    f.write(s)
  end
  print_success("Arduino Sketch File Complete\n")
end
